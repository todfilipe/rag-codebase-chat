...
|---|---|---|
| `GEMINI_API_KEY` | Só necessária no protótipo pré-migração; deixa de ser usada em `apps/web` depois da Fase 1 (passa a viver só no `rag-service`). | Protótipo |
| `GITHUB_TOKEN` | Idem — PAT interno para leitura de repos públicos. Migra para o `rag-service` na Fase 1. | Protótipo |
| `SUPABASE_URL` | URL do projeto Supabase. Mantém-se no `web` mesmo pós-migração, para Clerk/dashboard/billing lerem dados diretamente quando fizer sentido (não para RAG — isso passa a ser sempre via `rag-service`). | Protótipo |
